...
def get_coords(
    pin_number: int, pin_count: int, row_count: int, pitch: float, row_spacing: float
) -> Coord:
    """
...
def generate_pkg(config: Config) -> None:
        ...
        footprint.add_polygon(
            Polygon(
                ...
                vertices=[
                    Vertex(
                        Position(
                            coords.x - config.lead_width / 2 * sign,
                            coords.y + config.lead_width / 2,
                        ),
                        Angle(0),
                    ),
                    Vertex(
                    ...
                    Vertex(
                        Position(config.lead_span / 2 * sign, coords.y - config.lead_width / 2),
                        Angle(0),
                    ),
                    Vertex(
                        Position(config.lead_span / 2 * sign, coords.y + config.lead_width / 2),
    ...
    footprint.add_polygon(
        Polygon(
            ...
            vertices=[
                Vertex(
                    Position(-x_inside_body + triangle_offset, y_inside_body - triangle_offset),
                    Angle(0),
                ),
                Vertex(
                    Position(
                        -x_inside_body + triangle_offset,
                        y_inside_body - triangle_offset - triangle_size,
                    ),
                    Angle(0),
                ),
                Vertex(
                    Position(
                        -x_inside_body + triangle_offset + triangle_width,
                        y_inside_body - triangle_offset - triangle_size / 2,
                    ),
                    Angle(0),
                ),
                Vertex(
    ...
    def _create_outline(
        polygon_uuid: str, polygon_layer: str, polygon_offset: float, around_pads: bool
    ) -> Polygon:
        x_outline = body_bounds[0] + polygon_offset
        if around_pads:
            x_outline_extended = (
                abs(pin1.x) + config.pad_size[0] + config.pad_x_offset + polygon_offset
            )
        else:
...
def generate_dev(config: Config) -> None:
    ...
    for mpn in config.parts_mpn:
        device.add_part(Part(mpn=mpn, manufacturer=Manufacturer(config.parts_manufacturer or '')))
    ...
    configs = (
        [
            Config(
                ...
                pkg_categories=[
                    '92186130-e1a4-4a82-8ce9-88f4aa854195',
