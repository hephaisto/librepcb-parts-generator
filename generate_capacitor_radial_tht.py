...
def generate_pkg(
    ...
    if generate_3d_models:
        generate_3d(library, name, uuid_pkg, uuid_3d, diameter, height, pitch, lead_width)
...
def generate_3d(
    ...
    body_ring_cutout = (
        cq.Workplane('XZ', origin=(-body_ring_circle_radius, 0, body_ring_z))
    ...
    core = cq.Workplane('XY', origin=(0, 0, core_depth)).cylinder(
        height - 2 * core_depth, core_radius, centered=(True, True, False)
    )
    leg = (
        cq.Workplane('XY')
        .workplane(offset=(-core_depth - 1), invert=True)
        .cylinder(
            StepConstants.THT_LEAD_SOLDER_LENGTH + core_depth + 1,
            lead_width / 2,
    ...
    assembly.add_body(leg, 'leg-1', StepColor.LEAD_THT, location=cq.Location((-pitch / 2, 0, 0)))
    assembly.add_body(leg, 'leg-2', StepColor.LEAD_THT, location=cq.Location((pitch / 2, 0, 0)))
